...
Un défi majeur des MoE est l'de la spécialisation des experts (Expert Specialization Collapse), où le gateur envoie toutes les images vers le même expert, perdant ainsi son aspect de spécialisation. Pour contrer cela, nous avons implémenté une \textbf{perte auxiliaire} ($\mathcal{L}_{aux}$) ajoutée à la fonction de coût principale :
...
\begin{figure}[H]
    ...
    \caption{Comparaison de performance et de paramètres actifs}
    \label{fig:graph.png}
\end{figure}


\begin{itemize}
    \item \textbf{Evolution de la baseline :} Le modèle baseline semble suivre une courbe logarithmique: augmenter le nombre de paramètre rend le modèle plus robuste mais est de plus en plus gourmand.
    \item \textbf{Evolution des MoE :} TODO
    \item \textbf{Impact du Top-K :} TODO
    \item \textbf{Impact du du nombre d'expert :} TODO
    \item \textbf{Impact de l'aux :} TODO
\end{itemize}
...
\textbf{Analyse :}
\begin{itemize}
    \item \textbf{Absence de "Dead Experts" :} TODO
    \item \textbf{Équilibre cohérent :} TODO comparer avec résultats figure 2
\end{itemize}
